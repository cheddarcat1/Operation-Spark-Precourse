(function() {
  ...
  */
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    var uniqueArray = [];
    //Create an empty object to keep track of seen values 
    var seen = {};
    
    //If iterator is not provided
    if (!iterator) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      var value = array[i];
      //Apply the iterator function to the value
      ...
      if (!seen[transformed]) {
        ...
        //Mark the transformed value as seen
  ...
  _.map = function(collection, func) {
     // Create an empty array to store the result of applying the function
    var result = [];
    ...
    if (Array.isArray(collection)) {
      ...
      // Apply the function to the current element, index, and the whole collection
        result.push(func(collection[i], i, collection));
      }
      
      // Check if the collection is an object (and not null)
      ...
      for (var key in collection) {
        ...
        if (collection.hasOwnProperty(key)) {
          //Apply the function to the current property value, key, and the whole object
          result.push(func(collection[key], key, collection));
        }
      }
    }
    // Return the array containing the results of applying the function
    return result;
  }
  ...
  _.contains = function(collection, target) {
    ...
    // Return false if the target is not found in the collection
    return false;
  };
  ...
  _.delay = function(func, wait) {
   ...
   // Create an array to store arguments after func and wait
    ...
    for (var i = 2, length = arguments.length; i < length; i++) {
       // Push each argument to the argsAfterFuncAndWait array
  ...
  _.flatten = function(nestedArray, result) {
     ...
     // If result is not an array, initialize it as an empty array
    if (!Array.isArray(result)) {
      //result = an empty array;
      result = [];
    }
    
    //// Iterate through the elements in the nestedArray
    for (var i = 0; i < nestedArray.length; i++) {
      // Check if the current element is an array
      if (Array.isArray(nestedArray[i])) {
        // Recursively call flatten on the nested array
  ...
  _.intersection = function() {
    ...
    for (var i = 0; i < arrays[0].length; i++) {
      ...
      for (var j = 1; j < arrays.length; j++) {
        ...
        // Iterate through the elements in the current other array
        for (var k = 0; k < arrays[j].length; k++) {
          // Check if the element is equal to an element in the other array
        ...
        // If the element is not found in the current other array, exit the outer loop
        if (!found) {
          isShared = false;
          break;
        }
      }
      // If the element is shared among all arrays, add it to the result
      if (isShared) {
        result.push(element);
      }
    }
  ...
  */
  _.difference = function(array) {
    //Convert additional arguments into an array of otherArrays
    var otherArrays = [].slice.call(arguments, 1); 
    
    // Create an empty array to store the result
    var result = [];
    
    // Iterate through the elements in the main array
    for (var i = 0; i < array.length; i++) {
      var element = array[i];
      ...
      for (var j = 0; j < otherArrays.length; j++) {
        ...
        // If the element is found in any other array, exit the outer loop
        if (foundInOtherArrays) {
          break;
        }
      }
      // If the element is not found in any other array, add it to the result
      if (!foundInOtherArrays) {
        result.push(element);
      }
    }
    // Return the array containing the difference
    return result;
  };
...
function throttle(func, wait) {
    // Variables to track the last time the function was called and a timeout ID
    var lastCalled = 0;
    var timeoutId;
    var result;
    ...
    var throttled = function() {
        ...
        // Calculate the time remaining until the next valid call
        var remainingTime = wait - (now - lastCalled);
        
        // If the remaining time is zero or negative, it's time to call the function
        if (remainingTime <= 0) {
        ...
        // If we're still within the waiting period and no timeout is scheduled
        else if (!timeoutId) {
            // Schedule a timeout to call the function after the remaining time
            timeoutId = SetTimeout(function() {
                // Update the last called timestamp
